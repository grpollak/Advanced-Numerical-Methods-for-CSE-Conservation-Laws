\begin{defnbox}\nospacing
    \begin{defn}[Test function\hfill\tcblack{$\testfunc\in\sm_{C}^{1}\left(\R\times[0,T]\right)$}]\label{defn:test_function}
        \begin{minipage}[c]{0.5\textwidth}
            ...
            \imp{Idea}: use some test functions $\testfunc$ that has nicer properties than $u$ and shift the derivative from $u$ to $\testfunc$ by using integration by parts.
        \end{minipage}\hfill
...
\end{defnbox}
\begin{defnbox}\nospacing
    \begin{defn}[Weak Solutions\hfill\proofref{proof:defn:weak_solution}]\label{defn:weak_solution}\leavevmode\\
